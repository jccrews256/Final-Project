...
The code below reads in the data and applies the same processing steps used on the [exploratory data analysis page](https://jccrews256.github.io/Final-Project/EDA.html). 
...
rbind( tree_final_fit |> collect_metrics() |> mutate(model = "Class Tree") |> 
         select(model, everything()),
      rf_final_fit |> collect_metrics() |> mutate(model = "Random Forest") |> 
        select(model, everything()))
```
...
autoplot(conf_mat(data |> mutate(prediction = predictions), diabetes_binary, prediction), 
         type = "heatmap")
```
...
While not permanently hosted on the web, I have also produced an API that can be used to extract predictions from our final model fit to the full dataset, and the code for this API lives in the associated [GitHub repo](https://github.com/jccrews256/Final-Project/tree/main). If you deploy this API, happy predicting! 
